...
def view_item():
    ...
    c.execute('''
    select ID,ITEM,F_GOAL,TERM from ITEMS
    ''')
    records = c.fetchall()
...
def view_meth():
    ...
    c.execute('''
    select ID,METHOD, AVG_i, RISK_F from METHODS
    ''')
    records = c.fetchall()
...
def view_port():
    ...
    c.execute('''
    select a.start as Start, a.end as End, b.method ,a.Alloc*100 as Allocation, a.AVG_i as Avg_Int
    from R3 as a
    inner join Methods  as b
    on a.meth_id = b.id
    order by a.count, a.meth_id
    ''')
    records = c.fetchall()
    print(tabulate(list(records),headers=["Start","End","Method","% Allocation","Avg Interest"]))
...
edit_btn = Button(tab1, text="View Portfolio",command=view_port)
...
################## BEGINNING OF MAIN ALGORITYM ###########################
    ...
    while (lcnt != 0 and pcnt != 0):
        ...
        ##MODIFY UPDATE QRY BELOW FOR RETIREMENT
    ...
    update ITEMS
    set A_MAX = 
    round((select A.MIN_i/100 * (ITEMS.F_GOAL - ITEMS.P * power((1+A.MIN_i/100),ITEMS.TERM)) * 1 / (power((1+A.MIN_i/100),ITEMS.TERM) - 1)
    From stg AS A
    WHERE ITEMS.ID = A.ITEM_ID),2)
    WHERE ITEMS.ID IN (select ITEM_ID From stg);
    '''
